...
describe('Evaluation Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./evaluation-detail.component').then(m => m.EvaluationDetailComponent),
              resolve: { evaluation: () => of({ id: 12820 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load evaluation on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', EvaluationDetailComponent);

      // THEN
      expect(instance.evaluation()).toEqual(expect.objectContaining({ id: 12820 }));
    });
  });
